		...
		<div class="Post-body">
			<div class="Post-inner">
				<h2 class="PostHeaderIcon-wrapper">
					<span class="PostHeader">สถานะการแจ้งงาน</span>
				</h2>
				<div class="PostContent">
							...
							<tr class="<c:if test="${n.count%2 eq 1}">subTable</c:if>">
								...
								<td align="center"><c:choose>
										<c:when test="${inform.status eq 1}">
											<a href="editInform.htm?informId=${inform.id}">แจ้งงาน</a>
										</c:when>
										<c:when test="${inform.status eq 2}">
											<a href="sconfirmInform.htm?informId=${inform.id}">ยืนยัน</a>
										</c:when>
										<c:when test="${inform.status eq 3}">รอผู้บริการยืนยัน</c:when>
										<c:when test="${inform.status eq 4}">
											<a href="viewInform.htm?informId=${inform.id}">กำลังดำเนิน</a>
										</c:when>
									</c:choose></td>
							</tr>
						</c:forEach>
					...
					<p>
						* คลิก แจ้งงาน เพื่อแก้ไขรายละเอียดการแจ้งงาน<br /> * คลิก ยืนยัน
						เพื่อตอบรับวันเวลาที่นัดหมาย<br /> * คลิก กำลังดำเนิน
						เพื่อดูสถานะการดำเนินงานในปัจจุบัน<br />
					</p>
				</div>
