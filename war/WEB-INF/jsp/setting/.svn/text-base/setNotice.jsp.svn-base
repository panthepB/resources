						  ...
						  <div class="PostContent">
								    ...
								    <table width="100%" class="borderTable">
                                      ...
                                      <tr class="<c:if test="${n.count % 2 != 0}">subTable</c:if>">
                                        ...
                                        <td align="center">${notice.createDate} ${notice.createTime}</td>
                                      </tr>
                                      </c:forEach>
                                      <tr>
                                        <td colspan="6">
                                        <form:form method="POST" commandName="noticeForm" action="saveNotice.htm"><form:hidden path="id" />
                                          <table width="100%">
                                            ...
                                            <tr>
                                              ...
                                              <c:otherwise><input id="proceed" type="submit" class="styleButton" value="บันทึกการแก้ไขข้อมูล" onclick="goSubmit('save')" /> <input type="submit" class="styleButton" value="ลบข้อมูล" onclick="goSubmit('del');return confirm('คุณแน่ใจจะลบประกาศนี้หรือไม่');" /> <input type="button" class="styleButton" value="ยกเลิก" onclick="location.href='setNotice.htm'" /></c:otherwise>
                                              </c:choose></td>
                                            </tr>
                                          </table>
                                          </form:form>
							  	...
							  	<p>* คลิก หัวข้อประกาศ เพื่อแก้ไขข้อมูลของประกาศ <br /></p>									
							  </div>
