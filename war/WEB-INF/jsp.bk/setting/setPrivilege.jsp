			...
			<div class="Post-inner">
				...
				<div class="PostContent">
						...
						<c:forEach items="${positions}" var="position" varStatus="n">
							<tr class="<c:if test="${n.count % 2 != 0}">subTable</c:if>">
								...
								<td align="center"><c:choose>
										<c:when test="${position.status eq 1}">เปิด</c:when>
										<c:when test="${position.status eq 2}">ปิด</c:when>
									</c:choose></td>
							</tr>
						</c:forEach>
					</table>
					...
					<c:if test="${not empty privilegeForm}">
						<p>
							<form:form method="POST" commandName="privilegeForm"
								action="savePrivilege.htm">
								...
								<table width="65%" class="borderTable">
										...
										<td valign="top">
											<table width="100%">
												<c:forEach items="${groups}" var="group" varStatus="n">
													...
													<c:forEach items="${group.modules}" var="module"
														varStatus="n2">
														<tr>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
														...
														<c:forEach items="${module.tasks}" var="task"
															...
															<tr>
																<td>&nbsp;</td>
																<td>&nbsp;</td>
																<td><form:checkbox path="positionTasks"
																		value="${task.id}" label="${task.name}" /></td>
															</tr>
														</c:forEach>
													</c:forEach>
												</c:forEach>
									...
									<tr>
										<td colspan="2" class="subTable_button"><input
											type="submit" name="Button2" value="บันทึกข้อมูล"
											class="styleButton" /> <input type="reset" name="Submit22"
											value="เริ่มใหม่" class="styleButton" /> <input
											type="button" value="ยกเลิก" class="styleButton"
											onclick="location.href='setPrivilege.htm'" /></td>
									</tr>
								</table>
							</form:form>
						</p>
