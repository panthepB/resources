...
<%@ include file="/WEB-INF/jsp/headerTop.jsp" %>
	<title>ระบบจัดการการบริหารลูกค้า :: รายการการแจ้งงาน</title>
					...
					<div class="Post">
						<div class="Post-body">
							<div class="Post-inner">
								<h2 class="PostHeaderIcon-wrapper"><span class="PostHeader">รายการการแจ้งงาน</span></h2>
								<div class="PostContent">
										...
										<table width="100%" class="borderTable">
											...
											<tr class="<c:if test="${n.count%2 eq 1}">subTable</c:if>">
												...
												<td align="center"><c:choose>
													<c:when test="${inform.status eq 1}"><a href="confirmInform.htm?informId=${inform.id}">ยืนยัน</a></c:when>
													<c:when test="${inform.status eq 2}">รอผู้แจ้งยืนยัน</c:when>
													<c:when test="${inform.status eq 3}"><a href="confirmInform.htm?informId=${inform.id}">ยืนยัน</a></c:when>
													<c:when test="${inform.status eq 4}"><a href="setStatusInform.htm?informId=${inform.id}">กำลังดำเนิน</a></c:when>
													<c:when test="${inform.status eq 5}"><a href="viewInform.htm?informId=${inform.id}">เรียบร้อย</a></c:when>
												</c:choose></td>
											</tr>
											</c:forEach>
										</table>
									...
									<p>
										...
										* คลิก กำลังดำเนิน เพื่อบันทึกสถานะการดำเนินงาน<br />
										* คลิก เรียบร้อย เพื่อดูประวัติการดำเนินงานของการแจ้งงาน<br/>
									</p>
								</div>
