		...
		<div class="Post-body">
			<div class="Post-inner">
				...
				<div class="PostContent">
					<p>
						<form:form method="POST" commandName="informDetailForm"
							action="saveStatusInform.htm" name="form1">
							<form:hidden path="informDetailId" />
							...
							<table width="100%" class="borderTable">
								...
								<tr class="subHeadTable">
									...
									<td colspan="3" align="left" class="subTable">${inform.lastDayService}
										วัน</td>
								</tr>
								<tr class="subHeadTable">
										...
										<table width="100%">
											...
											<c:forEach items="${informDetails}" var="informDetail"
												...
												<tr class="subTable">
													...
													<td align="center"><c:choose>
															<c:when test="${informDetail.status eq 1}">
																<a
																	href="setStatusInform.htm?informId=${inform.id}&informDetailId=${informDetail.id}">นัดบริการ</a>
															</c:when>
											...
											<c:when test="${empty informDetailForm.informDetailId}">
												<tr class="subTable">
													...
													<td><input type="text" name="date" id="date"
														value="${informDetailForm.date}"
														dojoType="dijit.form.DateTextBox" /></td>
													<td class="subHeadTable">OnSite</td>
													<td><form:radiobutton path="onSite" label="ใช่"
															value="1" /> <form:radiobutton path="onSite" label="ไม่"
															value="0" /></td>
												</tr>
												<tr class="subTable">
													<td class="subHeadTable">เวลาเริ่มต้น</td>
													<td><input type="text" name="hourStart" id="hourStart"
														value="${informDetailForm.hourStart}"
														dojoType="dijit.form.NumberTextBox"
														constraints="{min:0,max:23}"
														style="width: 40px; text-align: center;"
														invalidMessage="กรุณากรอกข้อมูลให้ถูกต้อง" />:<input
														type="text" name="minuteStart" id="minuteStart"
														value="${informDetailForm.minuteStart}"
														dojoType="dijit.form.NumberTextBox"
														constraints="{min:0,max:59}"
														style="width: 40px; text-align: center;"
														invalidMessage="กรุณากรอกข้อมูลให้ถูกต้อง" /> น.</td>
													<td class="subHeadTable">ระยะเวลา<form:hidden
															path="hourEnd" />
														<form:hidden path="minuteEnd" /></td>
													<td colspan="3" align="left" class="subTable"><input
														type="text" name="dayService" id="dayService"
														value="${informDetailForm.dayService}"
														dojoType="dijit.form.NumberTextBox"
												...
												<tr class="subTable">
													...
													<td colspan="3"><form:select path="supporterId">
															<form:options items="${supporters}" itemLabel="name"
																itemValue="id" />
														</form:select></td>
												</tr>
												<tr class="subTable">
													<td class="subHeadTable">สถานะ</td>
													<td colspan="3"><form:radiobutton path="status"
															label="นัดหมาย" value="1" /></td>
												</tr>
											</c:when>
											<c:otherwise>
												<tr class="subTable">
												...
												<tr class="subTable">
													...
													<td><input type="text" name="hourStart" id="hourStart"
														...
														style="width: 40px; text-align: center;"
														invalidMessage="กรุณากรอกข้อมูลให้ถูกต้อง" /> น.</td>
													<td class="subHeadTable">เวลาสิ้นสุด<form:hidden
															path="dayService" /></td>
													<td><input type="text" name="hourEnd" id="hourEnd"
														value="${informDetailForm.hourEnd}"
														dojoType="dijit.form.NumberTextBox"
														constraints="{min:0,max:23}"
														style="width: 40px; text-align: center;"
														invalidMessage="กรุณากรอกข้อมูลให้ถูกต้อง" />:<input
														type="text" name="minuteEnd" id="minuteEnd"
														value="${informDetailForm.minuteEnd}"
														dojoType="dijit.form.NumberTextBox"
														constraints="{min:0,max:59}"
														style="width: 40px; text-align: center;"
														invalidMessage="กรุณากรอกข้อมูลให้ถูกต้อง" /> น.</td>
												...
												<tr class="subTable">
													<td class="subHeadTable">สถานะ</td>
													<td colspan="3"><form:radiobutton path="status"
															label="บริการแล้ว" value="2" /> <form:radiobutton
															path="status" label="เรียบร้อย" value="3" /></td>
												</tr>
											</c:otherwise>
										</c:choose>
										<tr>
											<td colspan="4" align="center" valign="top"
												...
												type="submit" class="styleButton" value="บันทึกการดำเนินงาน" />
												<input name="resetButton" type="reset" class="styleButton"
												value="เริ่มใหม่" /> <input name="cancel" type="button"
												class="styleButton" value="ยกเลิก"
												onclick="location.href='listInform.htm'" /></td>
										</tr>
									</c:when>
									<c:otherwise>
										<tr>
											<td colspan="4" align="center" valign="top"
												class="subTable_button"><input name="back"
												type="button" class="styleButton" value="กลับ"
												onclick="location.href='listInform.htm'" /></td>
										</tr>
									</c:otherwise>
								</c:choose>
