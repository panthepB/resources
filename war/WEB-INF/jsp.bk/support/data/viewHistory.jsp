		...
		<div class="Post-body">
			<div class="Post-inner">
				<h2 class="PostHeaderIcon-wrapper">
					<span class="PostHeader">ประวัติการบริการ ${customer.name}</span>
				</h2>
				<div class="PostContent">
					...
					<table width="100%" class="borderTable">
									...
									<c:forEach items="${contacts}" var="contact" varStatus="n">
										<tr class="<c:if test="${n.count%2 eq 1}">subTable</c:if>">
											...
											<td align="center"><c:if
													test="${not empty contact.desc}">
													<img src="<c:url value="/images/note.png"/>"
														title="${contact.desc}" />
												</c:if></td>
										</tr>
									</c:forEach>
						...
						<tr class="HeadTable">
							<td colspan="4" align="left" valign="top"><span
								class="style1">ข้อมูลการบริการ</span></td>
						</tr>
						<tr>
							<td colspan="4">
								<table width="100%">
									...
									<c:forEach items="${informs}" var="inform" varStatus="n">
										<tr class="<c:if test="${n.count%2 eq 1}">subTable</c:if>">
											...
											<td align="center"><a
												href="../service/viewInform.htm?informId=${inform.id}"><c:choose>
														<c:when test="${inform.status eq 1}">แจ้งงาน</c:when>
														<c:when test="${inform.status eq 2 or inform.status eq 3}">รอการยืนยัน</c:when>
														<c:when test="${inform.status eq 4}">กำลังดำเนิน</c:when>
						...
						<tr>
							<td colspan="4" align="center" valign="top"
								class="subTable_button"><input name="cancel" type="button"
								class="styleButton" value="กลับ"
								onclick="location.href='history.htm'" /></td>
						</tr>
					</table>
					</p>
				</div>
